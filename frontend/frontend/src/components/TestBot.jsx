...
export default function TestBot({ name, userId, closeChat, botId }) {
  return (
    <ModalProvider>
      <TestBotScreen name={name} userId={userId} closeChat={closeChat} botId={botId} />
    </ModalProvider>
  );
}

function TestBotScreen({ name, userId, closeChat, botId }) {
  const { state, dispatch } = useContext(ModalContext);
  useEffect(() => {
    // Dispatch actions to set initial state
    dispatch({ type: 'setName', payload: name });
    dispatch({ type: 'setUserId', payload: userId });
    dispatch({ type: 'setBotId', payload: botId });
  }, [name, userId, dispatch, botId]);
